function [out] = LP_admm_dual_test(model, opts)
%-------------------------------------------------------------------------
% This program tests ADMM for LP dual problem
% 
% Input:
...
if ~isfield(opts, 't1')  
    opts.t1 = 1/(8*(model.m+model.n)*mean(mean(model.obj))); end
...
model.tol = opts.tol;
out1 = LP_admm_dual(model);
% out1.t = opts.t2;
% out1.omg = opts.omg;
% out1.iter = opts.iter2;
% out2 = LP_smpf(out1);
